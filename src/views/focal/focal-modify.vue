		...
		<el-row :gutter="20">
			<el-col :span="24">
				<div class="grid-content bg-color">
					<el-button type="text" @click="route" style="font-size: 16px">焦点位管理</el-button><i class="el-icon-arrow-right"></i>修改焦点图</div>
			</el-col>
		</el-row>
		<el-col :span="24">
			<div class="el-had">
						...
						<el-col>
							<el-form-item label="封面:" prop="img" class="el-fo">
								<div class="upload">
									...
									<input type="file" accept="image/*" ref="upload" @change="onChange" style="position:absolute;clip:rect(0 0 0 0);">
								</div>
							</el-form-item>
						</el-col>
					</el-col>
...
<script>
	...
	import { getDropList, gatherInfo, gatherEdit } from '@/api/api'
	export default {
		data() {
			return {
				list: [],
		...
		created() {
			this.task['id'] = store.state.task
			this.getList()
			this.formLabel.img = 1
		},
		methods: {
			getList() {
				getDropList()
				...
				gatherInfo(this.task)
					.then(res => {
						...
						if(code == 1000) {
							this.formLabel = data
							this.FileUrl = this.formLabel.img
							this.formLabel.category = Number(this.formLabel.category)
							this.formLabel.img = 1
						}
			...
			onChange(event) {
				var _this = this;
				if(!this.formLabel.img instanceof Object) {
					this.formLabel.img = 1
				} else {
			...
			submitForm() {
				let start = this.formLabel.start_time;
				let end = this.formLabel.end_time;
				let data = new Date(start)
				let data1 = new Date(end)
